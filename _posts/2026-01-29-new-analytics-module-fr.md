...
Tout l'ancien module analytics, en fait, manque de flexibilité : dataviz codée "en dur", tables en base de données aussi, analyse d'usage strictement limitée aux données OGC. Et encore, limité aux services WxS (ne couvre pas les nouveaux standards OGC API).
...
- ils sont ensuite traités pour extraire les informations pertinentes et pour l'instant ne traitent que les usages OGC. Le résultat est stocké sous forme de table et de vues en base de données. 
    - Le traitement des données, c'est en fait le seul outil maison. Une CLI python qui, certes, pour l'instant ne couvre que les données OGC, mais est pensée pour être facile à enrichir de fonctionnalités pour aller au-délà des usages OGC. Python, pour être accessible à tous.
